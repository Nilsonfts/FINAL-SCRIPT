 ...
 * 
 * ВАЖНО: Для создания РАБОЧИЙ АМО используйте функции из файла 01_Главный_Скрипт_ЧИСТЫЙ.gs:
 * - runMergedAmoAnalysis() - основная функция с умным расчетом счетов
 * - runWorkingAmoClean() - чистая версия
 */

/**
 * БЫСТРЫЙ ЗАПУСК ЧИСТОЙ ВЕРСИИ AMO
 * Вызывает основную функцию из чистого скрипта
 */
function runCleanAmoAnalysis() {
  console.log('🔄 Переадресация на чистую версию AMO анализа...');
  
  try {
    // Вызываем основную функцию из чистого скрипта
    if (typeof runMergedAmoAnalysis === 'function') {
      runMergedAmoAnalysis();
    } else {
      console.error('❌ Функция runMergedAmoAnalysis не найдена. Убедитесь что файл 01_Главный_Скрипт_ЧИСТЫЙ.gs загружен в проект.');
    }
  } catch (error) {
    console.error('❌ Ошибка запуска чистой версии:', error);
    throw error;
  }
}
