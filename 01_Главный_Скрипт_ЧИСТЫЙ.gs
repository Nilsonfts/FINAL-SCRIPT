...
function onOpen() {
  ...
  ui.createMenu('📊 AMO АНАЛИТИКА')
    .addItem('🚀 Полный анализ (все отчеты)', 'runMergedAmoAnalysis')
    .addItem('⚡ Быстрый анализ (основное)', 'runQuickAnalysis')
    .addSeparator()
    .addItem('📋 Только РАБОЧИЙ АМО', 'runFullAnalyticsUpdate')
...
function runMergedAmoAnalysis() {
  ...
  try {
    // ШАГ 1: Создаем файл РАБОЧИЙ АМО из объединенных источников
    console.log('📊 ШАГ 1: Создание базового файла РАБОЧИЙ АМО...');
    buildMergedAmoFile();
...
function runQuickAnalysis() {
  ...
  try {
    // Создаем базовый файл РАБОЧИЙ АМО
