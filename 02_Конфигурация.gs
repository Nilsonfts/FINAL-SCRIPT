...
/**
 * Маппинг номеров колл-трекинга к источникам
 * Эти номера используются когда клиент ЗВОНИТ
 */
const CALL_TRACKING_MAPPING = {
  '78123172353': { source: 'call_site', name: 'Звонок с сайта', type: 'Звонки', channel: 'Основной сайт' },
  '78122428017': { source: 'call_ya_maps', name: 'Звонок с Яндекс Карт', type: 'Звонки', channel: 'Яндекс Карты' },
  '78122709071': { source: 'call_restoclub', name: 'Звонок с Рестоклаб', type: 'Звонки', channel: 'Рестоклаб' },
  '78123177149': { source: 'call_2gis', name: 'Звонок с 2Гис', type: 'Звонки', channel: '2Гис' },
  '78123177310': { source: 'call_smm', name: 'Звонок из соцсетей/Google', type: 'Звонки', channel: 'SMM' }
};

/**
 * Маппинг UTM источников (с июля 2025)
 * Эти метки используются когда клиент ОСТАВЛЯЕТ ЗАЯВКУ на сайте
 */
const UTM_SOURCE_DETAILED_MAPPING = {
  '2gis': { name: 'Заявка с 2Гис', type: 'Заявки', channel: '2Гис' },
  'geoadv_maps': { name: 'Заявка с поиска на картах', type: 'Заявки', channel: 'Карты' },
  'telegram': { name: 'Заявка из Telegram', type: 'Заявки', channel: 'Telegram' },
  'vk': { name: 'Заявка из ВКонтакте', type: 'Заявки', channel: 'ВКонтакте' },
  'ya_map': { name: 'Заявка с Яндекс Карт', type: 'Заявки', channel: 'Яндекс Карты' },
  'ya_map#booking': { name: 'Заявка с Яндекс Карт', type: 'Заявки', channel: 'Яндекс Карты' },
  'yandex': { name: 'Заявка из Яндекс Директ', type: 'Заявки', channel: 'Яндекс Директ' }
};

// Статусы для определения успешности (гость оплатил)
...
const CONFIG = {
  ...
  // Динамически определяемые колонки (правильная структура по умолчанию)
  WORKING_AMO_COLUMNS: null,

  // Установим правильную структуру по умолчанию
  initCorrectStructure: function() {
    this.WORKING_AMO_COLUMNS = this.WORKING_AMO_COLUMNS_CORRECT;
  },
...
try {
  // Сначала устанавливаем правильную структуру по умолчанию
  CONFIG.initCorrectStructure();
  
  // Затем определяем структуру таблицы (если лист существует)
  ...
  console.log('✅ Используется ПРАВИЛЬНАЯ структура колонок (41 столбец A-AO)');
  ...
  // В случае ошибки тоже используем правильную структуру
  CONFIG.WORKING_AMO_COLUMNS = CONFIG.WORKING_AMO_COLUMNS_CORRECT;
}
